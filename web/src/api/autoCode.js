...
const streamLLMRequest = async (url, data, options = {}) => {
  ...
  try {
    ...
    if (!contentType.includes('text/event-stream') || !response.body) {
      console.warn('[SSE] Response is not SSE. Content-Type:', contentType, '| Falling back to JSON parsing')
      const body = await parseFetchBody(response)
    ...
    console.debug('[SSE] Streaming mode started')
    ...
    while (!done) {
      ...
      // The last line may be incomplete; keep it for next chunk
      buffer = lines.pop() || ''
