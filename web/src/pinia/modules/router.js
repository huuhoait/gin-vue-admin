...
const KeepAliveFilter = (routes) => {
  routes &&
    routes.forEach((item) => {
      // If a child route uses keep-alive, the parent must also be keep-alive (otherwise it won't work).
      if (
...
export const useRouterStore = defineStore('router', () => {
  ...
  const setKeepAliveRouters = (history) => {
    ...
    // 1) add existing keepAlive config
    keepArrTemp.push(...keepAliveRoutersArr)
    if (config.keepAliveTabs) {
      history.forEach((item) => {
        // 2) force keep-alive for routes in history
        // Use routeMap + pathInfo to resolve component name
        ...
        // 3) if a child route is opened in tabs, parent should also be keepAlive
  ...
  const routeMap = {}
  // Fetch async routes from backend
