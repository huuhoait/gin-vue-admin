  ...
  <div>
    <div class="sticky top-0.5 z-10 flex space-x-2">
      <el-input
        ...
        class="flex-1"
        :placeholder="t('admin.superadmin.authority.apis.filter_name')"
      />
      <el-input
        v-model="filterTextPath"
        class="flex-1"
        :placeholder="t('admin.superadmin.authority.apis.filter_path')"
      />
...
<script setup>
  ...
  // exposed to parent: unified "enter then proceed" handler
  const enterAndNext = () => {
    authApiEnter()
  }
  ...
  const buildApiTree = (apis) => {
    ...
    for (const key in apiObj) {
      const treeNode = {
        ID: key,
        description: t('admin.superadmin.authority.apis.group_suffix', { name: key }),
        children: apiObj[key]
      }
  ...
  // submit the selected associations
  ...
  const authApiEnter = async () => {
    ...
    if (res.code === 0) {
      ElMessage({ type: 'success', message: t('admin.superadmin.authority.apis.save_success') })
    }
  }
