<template>
  <el-drawer
    v-model="drawer"
    :title="t('admin.layout.settings.title')"
    direction="rtl"
    :size="width"
    :show-close="false"
  ...
  >
    <template #header>
      <div class="flex items-center justify-between w-full px-6 py-4 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700">
        <h2 class="text-xl font-semibold gva-theme-text-main gva-theme-font">{{ t('admin.layout.settings.title') }}</h2>
        <el-button
          type="primary"
          size="small"
          class="reset-btn"
...
<script setup>
  ...
  // Labels resolved via computed so switching locale re-renders them.
  const tabs = computed(() => [
    { key: 'appearance', label: t('admin.layout.settings.tabs.appearance') },
    { key: 'layout', label: t('admin.layout.settings.tabs.layout') },
    { key: 'general', label: t('admin.layout.settings.tabs.general') }
  ])
  ...
  const saveConfig = async () => {
    ...
    if (res.code === 0) {
      ...
      ElMessage.success(t('admin.layout.settings.save_success'))
    }
  }
