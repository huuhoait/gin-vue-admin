<template>
  <div class="gva-tabs">
    ...
    <ul
      ...
      class="contextmenu"
    >
      <li @click="closeAll">{{ t('admin.layout.tabs.close_all') }}</li>
      <li @click="closeLeft">{{ t('admin.layout.tabs.close_left') }}</li>
      <li @click="closeRight">{{ t('admin.layout.tabs.close_right') }}</li>
      <li @click="closeOther">{{ t('admin.layout.tabs.close_other') }}</li>
    </ul>
  </div>
</template>
...
<script setup>
  ...
  /** Match sidebar/menu: translate i18n key in meta.title, then apply ${param} placeholders. */
  const tabTitle = (item) => {
    const raw = item?.meta?.title
    if (raw == null || raw === '') return ''
    return fmtTitle(t(raw), item)
  }
    ...
    historys.value = [
      {
        ...
        meta: {
          title: t('admin.layout.tabs.home')
        },
        query: {},
        params: {}
      }
  ...
  const initPage = () => {
    // global listener: close current page
    emitter.on('closeThisPage', () => {
      removeTab(getFmtString(route))
    })
    // global listener: close all pages
...
<style lang="scss" scoped>
  .contextmenu {
    @apply bg-white dark:bg-slate-900 w-36 m-0 py-2.5 px-0 border border-gray-200 text-sm shadow-md rounded absolute z-50 border-solid dark:border-slate-800;
  }
