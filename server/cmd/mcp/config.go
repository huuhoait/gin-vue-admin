...
func loadStandaloneConfig() (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("Read MCP configurationfailed: %w", err)
	}

	var cfg standaloneConfig
	if err := yaml.Unmarshal(content, &cfg); err != nil {
		return "", fmt.Errorf("Parse MCP configurationfailed: %w", err)
	}
...
func resolveConfigPath() (string, error) {
	...
	return "", errors.New("not found MCP IndependentconfigurationFile, PleaseAtCurrentDirectory, cmd/mcp DirectoryOrApproved -config / GVA_MCP_CONFIG Specify config.yaml")
}
...
func detectProjectRoot(startDir string) (string, error) {
	...
	return "", errors.New("NotAbleAutomaticIdentifyProjectRootDirectory, PleaseAt MCP configurationInset autocode.root")
}
...
func detectGoModule(goModPath string) (string, error) {
	...
	return "", errors.New("go.mod Innot found module Define")
}
