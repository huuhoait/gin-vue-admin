...
func (i *initUser) InitializeData(ctx context.Context) (next context.Context, err error) {
	...
	entities := []sysModel.SysUser{
		...
		},
		{
			...
			Password:    password,
			NickName:    "KYC Reviewer",
			HeaderImg:   "https://qmplusimg.henrongyi.top/1572075907logo.png",
			AuthorityId: 9100,
			Phone:       "17611111112",
			Email:       "kyc_reviewer@skyagent.local",
		},
		{
			UUID:        uuid.New(),
			Username:    "accountant",
			...
			AuthorityId: 9200,
			Phone:       "17611111113",
			Email:       "accountant@skyagent.local",
		},
		{
			UUID:        uuid.New(),
			Username:    "a303176530",
	...
	if !ok {
		return next, errors.Wrap(system.ErrMissingDependentContext, "Create [User-Permission] Associationfailed, permission table seed data not found")
	}
...
func (i *initUser) DataInserted(ctx context.Context) bool {
	...
	if errors.Is(db.Where("username = ?", "a303176530").
		Preload("Authorities").First(&record).Error, gorm.ErrRecordNotFound) { // Check if data exists
		return false
	}
