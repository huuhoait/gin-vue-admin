...
import (
	...
	email_response "github.com/huuhoait/gin-vue-admin/server/plugin/email/model/response"
	"github.com/huuhoait/gin-vue-admin/server/plugin/email/service"
...
func (s *EmailApi) EmailTest(c *gin.Context) {
	...
	if err != nil {
		global.GVA_LOG.Error("Send failed!", zap.Error(err))
		response.FailWithMessage("Send failed", c)
		return
	}
...
// @Security  ApiKeyAuth
// @Produce   application/json
// @Param     data  body      email_response.Email  true  "send emailrequired parameters"
// @Success   200   {string}  string                "{"success":true,"data":{},"msg":"Sent successfully"}"
