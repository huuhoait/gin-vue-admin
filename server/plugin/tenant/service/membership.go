...
func (s *membershipService) MembersOfTenant(tenantID uint) ([]MemberWithUser, error) {
	...
	err := global.GVA_DB.Table("gva_user_tenants AS ut").
		Select("ut.user_id AS user_id, ut.is_primary AS is_primary, ut.created_at AS created_at, u.username AS username, u.nick_name AS nick_name, u.email AS email, u.phone AS phone, u.enable AS enable").
