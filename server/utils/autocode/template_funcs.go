...
// GetTemplateFuncMap ReturnTemplateFunctionNumberMap, Used forAtTemplateInUse
...
func GenerateField(field systemReq.AutoCodeField) string {
	...
	// Forinttype, According toDataTypeLongdetermine concreteGotype, NotUsesizeTag
	...
	// by fieldtypeBuildDifferentofFieldDefine
	var result string
	...
	default:
		...
		// Forinttype, According toDataTypeLongdetermine concreteGotype
...
// Formatsearch conditionsStatement
func GenerateSearchConditions(fields []*systemReq.AutoCodeField) string {
    ...
    if info.%s != "" {
        db = db.Where("%s %s ?", info.%s)
				...
				condition = fmt.Sprintf(`
    if info.%s != "" {
        // TODO DatatypeForComplextype, PleaseAccording toBusinessNeedRequestSelfRowImplementComplextypeofQueryBusiness
    }`, field.FieldName)
			}
...
// FormatFrontendsearch conditions
func GenerateSearchFormItem(field systemReq.AutoCodeField) string {
	// start building form item
	result := fmt.Sprintf(`<el-form-item label="%s" prop="%s">
`, field.FieldDesc, field.FieldJson)

	// by fieldPropertyGenerateDifferentofInputIntype
	if field.FieldType == "bool" {
		result += fmt.Sprintf(`  <el-select v-model="searchInfo.%s" clearable placeholder="Please select">
`, field.FieldJson)
		result += `    <el-option key="true" label="Yes" value="true"></el-option>
`
		result += `    <el-option key="false" label="No" value="false"></el-option>
`
		result += `  </el-select>
`
		...
		result += fmt.Sprintf(`    <el-tree-select v-model="searchInfo.%s" placeholder="Please select%s" :data="%sOptions" style="width:100%%" filterable :clearable="%v" check-strictly %s></el-tree-select>
`,
		...
		result += fmt.Sprintf(`  <el-select %sv-model="searchInfo.%s" filterable placeholder="Please select%s" :clearable="%v">
`,
...
`
		...
		if field.FieldSearchType == "BETWEEN" || field.FieldSearchType == "NOT BETWEEN" {
			result += fmt.Sprintf(`  <el-input class="!w-40" v-model.number="searchInfo.start%s" placeholder="MinimumValue" />
`, field.FieldName)
			result += `  —
`
			result += fmt.Sprintf(`  <el-input class="!w-40" v-model.number="searchInfo.end%s" placeholder="MaximumValue" />
`, field.FieldName)
		} else {
			result += fmt.Sprintf(`  <el-input v-model.number="searchInfo.%s" placeholder="search conditions" />
`, field.FieldJson)
		}
...
`, field.FieldDesc)
			result += `      <el-tooltip content="SearchScopeYesStartDate(PackageInclude)toEndDate(NotPackageInclude)">
`
...
`
			result += `  </template>
`
			result += fmt.Sprintf(`<el-date-picker class="!w-380px" v-model="searchInfo.%sRange" type="datetimerange" range-separator="to"  start-placeholder="StartTime" end-placeholder="EndTime"></el-date-picker>`, field.FieldJson)
		} else {
			result += fmt.Sprintf(`<el-date-picker v-model="searchInfo.%s" type="datetime" placeholder="search conditions"></el-date-picker>`, field.FieldJson)
		}
	} else {
		result += fmt.Sprintf(`  <el-input v-model="searchInfo.%s" placeholder="search conditions" />
`, field.FieldJson)
	}
...
`, field.FieldDesc, field.FieldJson)
	...
	// HandleDifferentFieldtype
	if field.CheckDataSource {
		multipleAttr := ""
		if field.DataSource.Association == 2 {
			multipleAttr = " multiple"
		}
		result += fmt.Sprintf(`    <el-select%s v-model="formData.%s" placeholder="Please select%s" filterable style="width:100%%" :clearable="%v">
`,
			multipleAttr, field.FieldJson, field.FieldDesc, field.Clearable)
		result += fmt.Sprintf(`        <el-option v-for="(item,key) in dataSource.%s" :key="key" :label="item.label" :value="item.value" />
`,
...
`
		...
		switch field.FieldType {
		case "bool":
			result += fmt.Sprintf(`    <el-switch v-model="formData.%s" active-color="#13ce66" inactive-color="#ff4949" active-text="Yes" inactive-text="No" clearable ></el-switch>
`,
				field.FieldJson)

		case "string":
			if field.DictType != "" {
				result += fmt.Sprintf(`    <el-tree-select v-model="formData.%s" placeholder="Please select%s" :data="%sOptions" style="width:100%%" filterable :clearable="%v" check-strictly></el-tree-select>
`,
					field.FieldJson, field.FieldDesc, field.DictType, field.Clearable)
			} else {
				result += fmt.Sprintf(`    <el-input v-model="formData.%s" :clearable="%v" placeholder="please enter%s" />
`,
...
`, field.FieldJson)
		...
		case "json":
			result += fmt.Sprintf(`    // ThisFieldForjsonStructure, CanByFrontendSelfRowControlDisplayAndDataBindMode NeedBindjsonofkeyFor formData.%s AfterEndWillAccording tojsonoftypePerformSaveFetch
`, field.FieldJson)
...
`, field.FieldJson)
		...
		case "array":
			if field.DictType != "" {
				result += fmt.Sprintf(`    <el-select multiple v-model="formData.%s" placeholder="Please select%s" filterable style="width:100%%" :clearable="%v">
`,
...
`, field.FieldJson)
		...
		case "int":
			result += fmt.Sprintf(`    <el-input v-model.number="formData.%s" :clearable="%v" placeholder="please enter%s" />
`,
				field.FieldJson, field.Clearable, field.FieldDesc)

		case "time.Time":
			result += fmt.Sprintf(`    <el-date-picker v-model="formData.%s" type="date" style="width:100%%" placeholder="SelectDate" :clearable="%v" />
`,
...
`,
		...
		case "enum":
			result += fmt.Sprintf(`    <el-select v-model="formData.%s" placeholder="Please select%s" style="width:100%%" filterable :clearable="%v">
`,
				field.FieldJson, field.FieldDesc, field.Clearable)
...
func GenerateDefaultFormValue(field systemReq.AutoCodeField) string {
	// by fieldtypesureSetdefaultValue
	...
	case "string", "richtext":
		defaultValue = "''"
	case "int":
		if field.DataSource != nil { // check data sourceYesNoExists
	...
	// ReturnFormatAfterofdefaultValueString
	return fmt.Sprintf(`%s: %s,`, field.FieldJson, defaultValue)
}

// GenerateSearchField by fieldPropertyGenerateSearchStructureBodyInFieldDefine
func GenerateSearchField(field systemReq.AutoCodeField) string {
	var result string

	if field.FieldSearchType == "" {
		return "" // if there is noSearchtype, ReturnEmptyString
	}

	if field.FieldSearchType == "BETWEEN" || field.FieldSearchType == "NOT BETWEEN" {
		// GenerateScopeSearchField
		// time In this case
		if field.FieldType == "time.Time" {
