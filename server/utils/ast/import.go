		...
		if o1 {
			...
			for j := 0; j < len(v1.Specs); j++ {
				...
				if o2 && strings.HasSuffix(a.ImportPath, v2.Path.Value) {
					...
					} // if there is noimportDeclaration, Thendelete, if notdeleteThenWillOutAppearimport()
					break
				}
			}
		}
...
func (a *Import) Injection(file *ast.File) error {
	...
	} // if there is noimportDeclaration, ThenCreateOnePiece, MainRequireRequirePlaceAtNo.OnePiece
	return nil
}
