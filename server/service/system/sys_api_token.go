...
func (apiVersion *ApiTokenService) CreateApiToken(apiToken system.SysApiToken, days int) (string, error) {
	...
	if !hasAuth && user.AuthorityId != apiToken.AuthorityID {
		return "", errors.New("user does not have this role permission")
	}

	j := &utils.JWT{SigningKey: []byte(global.GVA_CONFIG.JWT.SigningKey)} // the only difference is the expiration time
	...
	claims := sysReq.CustomClaims{
		...
		BufferTime: int64(bf / time.Second), // buffer time
