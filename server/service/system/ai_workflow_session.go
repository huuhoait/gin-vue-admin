...
import (
	...
	systemResp "github.com/huuhoait/gin-vue-admin/server/model/system/response"
	"gorm.io/gorm"
)
...
func (s *aiWorkflowSession) Save(ctx context.Context, userID uint, info systemReq.SysAIWorkflowSessionUpsert) (session system.SysAIWorkflowSession, err error) {
	if userID == 0 {
		return session, errors.New("user not logged in")
	}
	if info.Tab != "analysis" && info.Tab != "workflow" {
		return session, errors.New("unsupported session type")
	}
