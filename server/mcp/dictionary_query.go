...
func (d *DictionaryQuery) New() mcp.Tool {
	return mcp.NewTool("query_dictionaries",
		mcp.WithDescription("QuerySystemInAllofDictionaryAndDictionaryProperty, Used forAIGenerateLogicWhenDonesolveCanUseofDictionaryChooseItem"),
		mcp.WithString("dictType",
			mcp.Description("CanChoose:Specifydictionary typePerformPreciseQuery, if notProvideThenReturnAllDictionary"),
		),
		mcp.WithBoolean("includeDisabled",
			mcp.Description("YesNoPackageIncludeDisabledofDictionaryAndDictionaryItem, defaultForfalse(OnlyReturnEnableof)"),
		),
		mcp.WithBoolean("detailsOnly",
			mcp.Description("YesNoOnlyReturnDictionaryDetailsInformation(NotPackageIncludeDictionaryBasic Info), defaultForfalse"),
		),
	)
}
...
func (d *DictionaryQuery) Handle(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	if detailsOnly {
		...
		return textResultWithJSON("", map[string]any{
			"success": true,
			"message": "QueryDictionaryDetailssucceeded",
			"total":   len(details),
			"details": details,
		})
	}
	...
	return textResultWithJSON("", DictionaryQueryResponse{
		Success:      true,
		Message:      "QueryDictionarysucceeded",
		Total:        len(result),
		Dictionaries: result,
	})
}
