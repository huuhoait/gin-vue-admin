...
func doUpstream[T any](ctx context.Context, method, endpoint string, query url.Values, body any) (*upstreamEnvelope[T], error) {
	...
	if token == "" {
		return nil, fmt.Errorf("missing MCP auth header: %s", configuredAuthHeader())
	}

	endpoint = strings.TrimSpace(endpoint)
	if endpoint == "" {
		return nil, fmt.Errorf("UppertourAPIpathcannot be empty")
	}
	...
	if err != nil {
		return nil, fmt.Errorf("Buildupstream requestAddressfailed: %w", err)
	}
	if len(query) > 0 {
	...
	if body != nil {
		...
		if err != nil {
			return nil, fmt.Errorf("Serializeupstream requestfailed: %w", err)
		}
	...
	if err != nil {
		return nil, fmt.Errorf("Createupstream requestfailed: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("RequestUppertourServicefailed: %w", err)
	}
	defer resp.Body.Close()

	rawBody, err := io.ReadAll(resp.Body)
	if err != nil {
		return nil, fmt.Errorf("ReadUppertourResponsefailed: %w", err)
	}

	var result upstreamEnvelope[T]
	if len(rawBody) > 0 {
		if err := json.Unmarshal(rawBody, &result); err != nil {
			return nil, fmt.Errorf("ParseUppertourResponsefailed: %w", err)
		}
	}
	...
	if resp.StatusCode >= http.StatusBadRequest {
		...
		return nil, fmt.Errorf("upstream requestfailed, statusCode: %d", resp.StatusCode)
	}
	...
	if result.Code != 0 {
		...
		return nil, fmt.Errorf("upstream requestfailed, BusinessCode: %d", result.Code)
	}
